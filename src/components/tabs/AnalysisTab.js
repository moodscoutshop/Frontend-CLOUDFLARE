    ...
    <div className="py-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 lg:gap-8 mb-6">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="w-10 h-10 bg-gradient-to-br from-[#B62AD9] to-[#2F80FA] rounded-lg flex items-center justify-center">
            <Sparkles className="w-5 h-5 text-white" />
          </div>
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-[#1D1F20]">AI Analysis</h2>
            <p className="text-sm text-[#5D5F60]">Understanding your aesthetic</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          {analysisData?.model_used && (
            <span className="text-xs text-[#5D5F60]">Model: {analysisData.model_used}</span>
          )}
        </div>
      </div>
      ...
      {isLoading && !analysisData && (
        <div className="bg-[#FDFDF8] p-6 rounded-lg shadow-md border border-[#D4CFC0] animate-pulse">
      ...
      {/* Analysis Content */}
      {analysisData && (
        <div className="bg-white rounded-xl shadow-lg border border-[#D4CFC0] overflow-hidden">
          {/* Gradient Header Bar */}
          <div className="h-2 bg-gradient-to-r from-[#B62AD9] via-[#2F80FA] to-[#36C46F]"></div>
          ...
          <div className="p-5 sm:p-6">
                    ...
                    {analysisData.color_palette.map((color, idx) => (
                      <span
                        ...
                        className="
                          ...
                          bg-[#2F80FA]/12 text-[#2F80FA] border border-[#2F80FA]/30
            ...
            {keywordsModified && editedKeywords.length > 0 && (
              <div className="mt-6 pt-4 border-t border-[#E0DCCE] flex items-center justify-between gap-4">
      ...
      {!isLoading && !analysisData && (
        <div className="text-center py-12">
          <div className="w-14 h-14 bg-[#EEEFE9] rounded-lg flex items-center justify-center mx-auto mb-4">
            <Sparkles className="w-7 h-7 text-[#5D5F60]" />
          </div>
          <h3 className="text-lg font-semibold text-[#3D3F40] mb-2">Analysis Pending</h3>
          <p className="text-[#5D5F60]">AI analysis will appear here once processing is complete.</p>
        </div>
      )}
    </div>
  );
